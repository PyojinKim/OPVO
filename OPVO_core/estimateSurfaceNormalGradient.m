function [surfaceNormalVector, surfacePixelPoint] = estimateSurfaceNormalGradient(imageCur, depthCur, cam, optsMWO)
...
rawDepthCur(rawDepthCur < 0.5 | rawDepthCur > 8) = -1000;
...
for v = 2:(imageHeight-1)
    for u = 2:(imageWidth-1)
        ...
        validIdx = (testValues >= 0.5) & (testValues <= 8);
        if (sum(validIdx) == 5)
            tangeMask(v,u) = 1;
